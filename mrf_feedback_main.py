...
function		: main script performing feedback control of product photoluminescent properties
				  in millifluidic reactor
******************************************************************************
"""
...
if __name__ == "__main__":
	try:
		...
		for _ in range(180): # long enough for 6 hr reaction
			t_now = datetime.datetime.now()
			
			# assign file names
			time_str = str(t_now.hour).zfill(2)+str(t_now.minute).zfill(2)
			npy_extension = time_str+'_'+str(p1_set)+'_'+str(p3_set)+'_'+str(p4_set)+'.npy'
			out_extension = time_str+'_'+str(p1_set)+'_'+str(p3_set)+'_'+str(p4_set)+'.out'
			pl_name = 'PL_'+ npy_extension
			abs_name = 'Abs_'+ npy_extension 
			ir_name = 'IR_'+ npy_extension
			peak_name = 'peaks_'+ out_extension
			fwhm_name = 'fwhms_'+ out_extension
			
			
			# define then start ir_mux thread and pl_abs thread
			t1 = threading.Thread(name = 'IR', target = ir_mux, args = ([ir_name]))
			t2 = threading.Thread(name = 'PL-UV-vis', target = pl_abs, 
				args = (pl_name, abs_name))
